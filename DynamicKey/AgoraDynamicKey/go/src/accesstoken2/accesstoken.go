...
func (serviceChat *ServiceChat) Pack(w io.Writer) (err error) {
    ...
    if err != nil {
        panic(fmt.Sprintf("pack service failed: %v", err.Error()))
    }
...
func (serviceChat *ServiceChat) UnPack(r io.Reader) (err error) {
    ...
    if err != nil {
        panic(fmt.Sprintf("unpack service failed: %v", err.Error()))
    }
...
func (accessToken *AccessToken) Build() (res string, err error) {
    ...
    serviceTypes := []uint16{ServiceTypeRtc, ServiceTypeRtm, ServiceTypeStreaming, ServiceTypeFpa, ServiceTypeChat}
