...
func Test_BuildToken(t *testing.T) {
	...
	accesstoken.AssertEqual(t, 1, len(accessToken.GetServices(accesstoken.ServiceTypeConvoAI)))
	accesstoken.AssertEqual(t, 0, len(accessToken.GetServices(accesstoken.ServiceTypeConvoAI)[0].(*accesstoken.ServiceConvoAI).Privileges))
}
