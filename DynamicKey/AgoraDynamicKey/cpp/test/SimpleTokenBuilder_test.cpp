...
class SimpleTokenBuilder_test : public testing::Test {
 protected:
  ...
  void testSimpleTokenBuilderWithIntUid() {
    {
      std::string token = SimpleTokenBuilder::buildTokenWithUid(
          appID, appCertificate, channelName, uid, UserRole::Role_Subscriber,
          expiredTs);
      AccessToken parser;
      ...
      EXPECT_EQ(AccessToken::GenerateSignature(appCertificate, appID,
                                               channelName, std::to_string(uid),
  ...
  void testSimpleTokenBuilderWithZero() {
    std::string token = SimpleTokenBuilder::buildTokenWithUid(
        appID, appCertificate, channelName, 0, UserRole::Role_Attendee,
        expiredTs);
    AccessToken parser;
    parser.FromString(token);
    EXPECT_EQ(parser.app_id_, appID);
    ...
    EXPECT_EQ(parser.message_.messages[AccessToken::kPublishDataStream],
              expiredTs);
    EXPECT_EQ(AccessToken::GenerateSignature(appCertificate, appID, channelName,
                                             "", parser.message_raw_content_),
              parser.signature_);
  }
  ...
  void testSimpleTokenBuilderWithStringUid() {
    {
      std::string token = SimpleTokenBuilder::buildTokenWithUserAccount(
          appID, appCertificate, channelName, uidStr, UserRole::Role_Subscriber,
          expiredTs);
      AccessToken parser;
      parser.FromString(token);
      EXPECT_EQ(parser.app_id_, appID);
      ...
      EXPECT_EQ(parser.message_.messages[AccessToken::kPublishDataStream], 0);
      EXPECT_EQ(
          AccessToken::GenerateSignature(appCertificate, appID, channelName,
                                         uidStr, parser.message_raw_content_),
          parser.signature_);
    }
    {
      std::string token = SimpleTokenBuilder::buildTokenWithUserAccount(
          appID, appCertificate, channelName, uidStr, UserRole::Role_Attendee,
          expiredTs);
      AccessToken parser;
      parser.FromString(token);
      EXPECT_EQ(parser.app_id_, appID);
