...
int main(int argc, char const *argv[]) {
  ...
  auto room_user_token = ApaasTokenBuilder::BuildRoomUserToken(app_id, app_certificate, room_uuid, user_uuid, role, expiration_in_seconds);
  std::cout << "Apaas room user token:" << room_user_token << std::endl;

  auto user_token = ApaasTokenBuilder::BuildUserToken(app_id, app_certificate, user_uuid, expiration_in_seconds);
  std::cout << "Apaas user token:" << user_token << std::endl;

  auto app_token = ApaasTokenBuilder::BuildAppToken(app_id, app_certificate, expiration_in_seconds);
  std::cout << "Apaas app token:" << app_token << std::endl;
