...
class RtcTokenBuilder2 {
 public:
  ...
  static std::string BuildTokenWithUid(const std::string& app_id, const std::string& app_certificate, const std::string& channel_name, uint32_t uid,
  ...
  /**
   ...
   @return The new Token.
   */
  static std::string BuildTokenWithUserAccount(const std::string& app_id, const std::string& app_certificate, const std::string& channel_name,
   ...
   *
   * The privileges for publishing audio, video, and data streams in an RTC
   * channel apply only if you have enabled co-host authentication.
   *
   * A user can have multiple privileges. Each privilege is valid for a maximum
   * of 24 hours. The SDK triggers the onTokenPrivilegeWillExpire and
   * onRequestToken callbacks when the token is about to expire or has expired.
   * The callbacks do not report the specific privilege affected, and you need
   * to maintain the respective timestamp for each privilege in your app logic.
   * After receiving the callback, you need to generate a new token, and then
   * call renewToken to pass the new token to the SDK, or call joinChannel to
   * re-join the channel.
   *
   * @note
   * Agora recommends setting a reasonable timestamp for each privilege
   * according to your scenario. Suppose the expiration timestamp for joining
   * the channel is set earlier than that for publishing audio. When the token
   * for joining the channel expires, the user is immediately kicked off the RTC
   * channel and cannot publish any audio stream, even though the timestamp for
   * publishing audio has not expired.
   *
   * @param app_id The App ID of your Agora project.
   * @param app_certificate The App Certificate of your Agora project.
   * @param channel_name The unique channel name for the Agora RTC session in
   * string format. The string length must be less than 64 bytes. The channel
   * name may contain the following characters:
   ...
   * - "!", "#", "$", "%", "&", "(", ")", "+", "-", ":", ";", "<", "=", ".",
   * ">", "?", "@", "[", "]", "^", "_", " {", "}", "|", "~", ",".
   * @param uid The user ID. A 32-bit unsigned integer with a value range from 1
   * to (2^32 - 1). It must be unique. Set uid as 0, if you do not want to
   * authenticate the user ID, that is, any uid from the app client can join the
   * channel.
   * @param token_expire represented by the number of seconds elapsed since now.
   ...
   */
  static std::string BuildTokenWithUid(const std::string& app_id, const std::string& app_certificate, const std::string& channel_name, uint32_t uid,
                                       uint32_t token_expire, uint32_t join_channel_privilege_expire = 0, uint32_t pub_audio_privilege_expire = 0,
                                       uint32_t pub_video_privilege_expire = 0, uint32_t pub_data_stream_privilege_expire = 0);
   ...
   */
  static std::string BuildTokenWithUserAccount(const std::string& app_id, const std::string& app_certificate, const std::string& channel_name,
                                               const std::string& user_account, uint32_t token_expire, uint32_t join_channel_privilege_expire = 0,
                                               uint32_t pub_audio_privilege_expire = 0, uint32_t pub_video_privilege_expire = 0,
...
inline std::string RtcTokenBuilder2::BuildTokenWithUid(const std::string& app_id, const std::string& app_certificate, const std::string& channel_name,
                                                       uint32_t uid, UserRole role, uint32_t token_expire, uint32_t privilege_expire) {
  std::string account;
  if (uid != 0) {
    account = std::to_string(uid);
  }
  return RtcTokenBuilder2::BuildTokenWithUserAccount(app_id, app_certificate, channel_name, account, role, token_expire, privilege_expire);
}

inline std::string RtcTokenBuilder2::BuildTokenWithUserAccount(const std::string& app_id, const std::string& app_certificate, const std::string& channel_name,
                                                               const std::string& user_account, UserRole role, uint32_t token_expire,
                                                               uint32_t privilege_expire) {
  std::unique_ptr<Service> service(new ServiceRtc(channel_name, user_account));
...
inline std::string RtcTokenBuilder2::BuildTokenWithUid(const std::string& app_id, const std::string& app_certificate, const std::string& channel_name,
                                                       uint32_t uid, uint32_t token_expire, uint32_t join_channel_privilege_expire,
                                                       uint32_t pub_audio_privilege_expire, uint32_t pub_video_privilege_expire,
                                                       uint32_t pub_data_stream_privilege_expire) {
  std::string account;
  if (uid != 0) {
    account = std::to_string(uid);
  }
  return RtcTokenBuilder2::BuildTokenWithUserAccount(app_id, app_certificate, channel_name, account, token_expire, join_channel_privilege_expire,
                                                     pub_audio_privilege_expire, pub_video_privilege_expire, pub_data_stream_privilege_expire);
}

inline std::string RtcTokenBuilder2::BuildTokenWithUserAccount(const std::string& app_id, const std::string& app_certificate, const std::string& channel_name,
                                                               const std::string& user_account, uint32_t token_expire, uint32_t join_channel_privilege_expire,
                                                               uint32_t pub_audio_privilege_expire, uint32_t pub_video_privilege_expire,
                                                               uint32_t pub_data_stream_privilege_expire) {
  std::unique_ptr<Service> service(new ServiceRtc(channel_name, user_account));
  service->AddPrivilege(ServiceRtc::kPrivilegeJoinChannel, join_channel_privilege_expire);
  service->AddPrivilege(ServiceRtc::kPrivilegePublishAudioStream, pub_audio_privilege_expire);
  service->AddPrivilege(ServiceRtc::kPrivilegePublishVideoStream, pub_video_privilege_expire);
  service->AddPrivilege(ServiceRtc::kPrivilegePublishDataStream, pub_data_stream_privilege_expire);
...
inline std::string RtcTokenBuilder2::BuildTokenWithRtm(const std::string& app_id, const std::string& app_certificate, const std::string& channel_name,
                                                       const std::string& user_account, UserRole role, uint32_t token_expire, uint32_t privilege_expire) {
  std::unique_ptr<Service> rtc_service(new ServiceRtc(channel_name, user_account));
  AccessToken2 token(app_id, app_certificate, 0, token_expire);

  rtc_service->AddPrivilege(ServiceRtc::kPrivilegeJoinChannel, privilege_expire);
  if (role == UserRole::kRolePublisher) {
    rtc_service->AddPrivilege(ServiceRtc::kPrivilegePublishAudioStream, privilege_expire);
    rtc_service->AddPrivilege(ServiceRtc::kPrivilegePublishVideoStream, privilege_expire);
    rtc_service->AddPrivilege(ServiceRtc::kPrivilegePublishDataStream, privilege_expire);
  }
