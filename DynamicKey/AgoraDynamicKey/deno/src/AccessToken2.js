        ...
        for (let i = 0; i < resourceTypeCount; i++) {
            ...
            for (let j = 0; j < permissionCount; j++) {
                ...
                for (let k = 0; k < resourceCount; k++) {
                    resources.push(reader.getString())
                }
...
class AccessToken2 {
    ...
    from_string(origin_token) {
        ...
        try {
            ...
            const buffer = Buffer.from(inflate(Buffer.from(origin_content, 'base64')))
            const bufferReader = new ReadByteBuf(buffer)

            this.__signature = bufferReader.getBytes()
    ...
    verifySignature(appCertificate) {
        ...
        const signature = Buffer.from(encodeHMac(this.__signing(appCertificate), this.__signing_info))
        return constantTimeEqual(this.__signature, signature)
    }
}
...
var ReadByteBuf = function (bytes) {
    ...
    that.getBytes = function () {
        ...
        if (that.position + len > that.buffer.length) {
            throw new RangeError('invalid packed byte length')
        }
