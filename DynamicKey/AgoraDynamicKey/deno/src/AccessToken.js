...
import {UINT32} from './cuint.js'
export const version = "006";
var randomInt = Math.floor(Math.random() * 0xFFFFFFFF);
const VERSION_LENGTH = 3;
const APP_ID_LENGTH = 32;

export const AccessToken = function (appID, appCertificate, channelName, uid) {
    let token = this;
    this.appID = appID;
    ...
    this.build = function () {
        ...
        var crc_channel = UINT32(crc32(token.channelName), 16).and(UINT32(0xffffffff)).toNumber();
        var crc_uid = UINT32(crc32(token.uid), 16).and(UINT32(0xffffffff)).toNumber();
