...
class RtcTokenBuilder2 {
     ...
     *                          set tokenExpire as 600(seconds). This is the whole token expiration.
     *                          The whole token is invalid after this time, even if a privilege expiration
     *                          time is later.
    ...
    fun buildTokenWithUserAccount(
        ...
        val serviceRtc = AccessToken2.ServiceRtc(channelName, account)
     ...
     * @param tokenExpire                  represented by the number of seconds elapsed since now. For
     *                                     example, to access the Agora Service within 10 minutes after
     *                                     the token is generated, set tokenExpire as 600(seconds). This is
     *                                     the whole token expiration. The whole token is invalid after
     ...
     *                                     For example, to stay in the channel for 10 minutes, set
     *                                     joinChannelPrivilegeExpire as 600(seconds). This value must not
     *                                     exceed the token expiration value; otherwise, the privilege is
     *                                     limited by the token expiration time.
     * @param pubAudioPrivilegeExpire      represented by the number of seconds elapsed since now.
     *                                     For example, to enable audio publishing for 10 minutes, set
     *                                     pubAudioPrivilegeExpire as 600(seconds). This value must not
     *                                     exceed the token expiration value; otherwise, the privilege is
     *                                     limited by the token expiration time.
     * @param pubVideoPrivilegeExpire      represented by the number of seconds elapsed since now.
     *                                     For example, to enable video publishing for 10 minutes, set
     *                                     pubVideoPrivilegeExpire as 600(seconds). This value must not
     *                                     exceed the token expiration value; otherwise, the privilege is
     *                                     limited by the token expiration time.
     * @param pubDataStreamPrivilegeExpire represented by the number of seconds elapsed since now.
     *                                     For example, to enable data stream publishing for 10 minutes,
     *                                     set pubDataStreamPrivilegeExpire as 600(seconds). This value
     *                                     must not exceed the token expiration value; otherwise, the
    ...
    fun buildTokenWithRtm(
        ...
        val serviceRtm = AccessToken2.ServiceRtm(account)
     ...
     *                                     you want to access the Agora Service within 10 minutes after the token is generated,
     *                                     set rtcTokenExpire as 600(seconds). This is the whole token
     ...
     *                                     you want to access the Agora Service within 10 minutes after the token is generated,
     *                                     set rtmTokenExpire as 600(seconds). This value must not exceed
     *                                     the RTC token expiration value; otherwise, the RTM login
     *                                     privilege is limited by the token expiration time.
     * @return The RTC and RTM token.
     */
    fun buildTokenWithRtm2(
        appId: String,
        appCertificate: String,
        channelName: String,
        rtcAccount: String,
