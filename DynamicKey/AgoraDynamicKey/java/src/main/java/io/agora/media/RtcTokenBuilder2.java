...
public class RtcTokenBuilder2 {
     ...
     *                          set tokenExpire as 600(seconds). This is the whole token expiration. The whole
     *                          token is invalid after this time, even if a privilege expiration time is
     *                          later.
     ...
     *                                     tokenExpire as 600(seconds). This is the whole token expiration.
     *                                     The whole token is invalid after this time, even if a privilege
     *                                     expiration time is later.
     * @param joinChannelPrivilegeExpire   represented by the number of seconds elapsed since now.
     *                                     If, for example, you want to join channel and expect stay in the
     *                                     channel for 10 minutes, set joinChannelPrivilegeExpire as
     *                                     600(seconds). This value must not exceed the token expiration
     *                                     value; otherwise, the privilege is limited by the token expiration
     *                                     time.
     * @param pubAudioPrivilegeExpire      represented by the number of seconds elapsed since now.
     *                                     If, for example, you want to enable publish audio privilege for 10
     *                                     minutes, set pubAudioPrivilegeExpire as 600(seconds). This value
     ...
     *                                     If, for example, you want to enable publish video privilege for 10
     *                                     minutes, set pubVideoPrivilegeExpire as 600(seconds). This value
     *                                     must not exceed the token expiration value; otherwise, the
     *                                     privilege is limited by the token expiration time.
     * @param pubDataStreamPrivilegeExpire represented by the number of seconds elapsed since now.
     *                                     If, for example, you want to enable publish data stream privilege
     *                                     for 10 minutes, set pubDataStreamPrivilegeExpire as 600(seconds).
     *                                     This value must not exceed the token expiration value; otherwise,
     ...
     *                                     expiration. The whole token is invalid after this time, even if a
     ...
     *                                     you want to access the Agora Service within 10 minutes after the token is generated,
     *                                     set rtmTokenExpire as 600(seconds). This value must not exceed the
     *                                     RTC token expiration value; otherwise, the RTM login privilege is
